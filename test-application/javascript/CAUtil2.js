...
exports.registerAndEnrollUser = async (caClient, wallet1 , wallet2 , userId ,userpw,useremail,userrole) => {
	try {
		...
		const adminIdentity = await wallet2.get(adminUserId);
		if (!adminIdentity) {
			return;
		}
       
		// build a user object for authenticating with the CA
		const provider = wallet2.getProviderRegistry().getProvider(adminIdentity.type);
		...
		await wallet2.put(userId, x509Identity,userpw,useremail,userrole);
	} catch (error) {
